...
async function ensureTargetSchema(target) {
  await target.query(`
    CREATE TABLE IF NOT EXISTS public.soul_catalog_entries (
      id text PRIMARY KEY,
  ...
  await target.query(`
    CREATE TABLE IF NOT EXISTS public.soul_marketplace_audit (
  ...
  await target.query(`
    CREATE TABLE IF NOT EXISTS public.soul_marketplace_drafts (
      wallet_address text NOT NULL,
...
async function main() {
  ...
  try {
    ...
    await copyTable({
      source,
      target,
      sourceTable: 'public.soul_catalog_entries',
      targetTable: 'public.soul_catalog_entries',
    ...
    await copyTable({
      source,
      target,
      sourceTable: 'public.soul_marketplace_audit',
      targetTable: 'public.soul_marketplace_audit',
      columns: [
        'id',
        'at',
        'event',
    ...
    await copyTable({
      source,
      target,
      sourceTable: 'public.soul_marketplace_drafts',
      targetTable: 'public.soul_marketplace_drafts',
      columns: [
        'wallet_address',
        'draft_id',
        'status',
    ...
    await syncIdentitySequence(target, 'public.soul_marketplace_audit', 'id');
